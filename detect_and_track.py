...
# import csv
# from reid_utils import ReIDMatcher  # Import ReIDMatcher from reid_utils.py

# # Paths
# VIDEO_DIR = r'D:\Projects\CV-Project\CV_Assessment'  # Change to your video folder path
# OUTPUT_DIR = 'output'
# Path(OUTPUT_DIR).mkdir(exist_ok=True)

# # Load YOLOv8 model
# model = YOLO('yolov8m.pt')  # Change to yolov8s.pt for better accuracy

# # Initialize Deep SORT tracker
# tracker = DeepSort(max_age=30)

# # Initialize ReID Matcher for global ID assignment
# matcher = ReIDMatcher(similarity_thresh=0.75)  # Use similarity threshold for ID matching

# # CSV structure to save tracking data
...
#         dets_for_sort = []
#         for det,class_ids in zip(detections, class_ids):
#             if int(class_ids) != 0:  # Only consider person class
#                 continue
#             x1, y1, x2, y2 = det[:4]
#             conf = det[4] if len(det) >= 5 else 1.0
#             dets_for_sort.append(([x1, y1, x2 - x1, y2 - y1], conf, 'person'))
...
#             # Crop and compute embedding + global ID using ReID matcher
#             crop = frame[y1:y2, x1:x2]
#             global_id = matcher.assign_id(crop)

#             # Save result to CSV
#             csv_rows.append((global_id, video_name, frame_idx, x1, y1, w, h))
...
# VIDEO_DIR = r'D:\Projects\CV-Project\CV_Assessment'
# OUTPUT_DIR = 'output'
# Path(OUTPUT_DIR).mkdir(exist_ok=True)

# model = YOLO('yolov8m.pt')  # ⬅️ Upgraded from yolov8n for better accuracy

# tracker = DeepSort(max_age=20)
# matcher = ReIDMatcher(similarity_thresh=0.65)  # ⬅️ Slightly relaxed for varied angles
...
#             global_id = matcher.assign_id(crop)  # ⬅️ Use ReID to ensure same-ID
